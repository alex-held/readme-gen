...
import pinfo from "./project-infos";
import PackageInfo from "./project-infos";
...
describe('pinfo', () => {
  describe('getProjectInfos', () => {
    ...
    it('should return correct infos', async () => {
      const packgeJsonInfos : PkgJson = PackageJson.ForceCreateUnsafeFromInterface( {
      ...
      utils.getPackageJson = jest.fn().mockReturnValueOnce(Promise.resolve(packgeJsonInfos))
      childProcess.execSync.mockReturnValue(
        'https://github.com/kefranabg/readme-md-generator.git'
      )

      const projectInfos = await pinfo.getProjectInfos()

      expect(projectInfos).toEqual(PackageJson.ForceCreateUnsafeFromInterface( {
        name: 'readme-md-generator',
        description: 'CLI that generates beautiful README.md files.',
        version: '0.1.3',
        author: 'Franck Abgrall',
    ...
    it('should return correct infos when package.json is not defined', async () => {
      utils.getPackageJson =jest.fn().mockReturnValueOnce(Promise.resolve(undefined))
      childProcess.execSync.mockReturnValue(
        'https://github.com/kefranabg/readme-md-generator.git'
      )
    ...
    it('should return correct infos when lock file is found', async () => {
      ...
      utils.getPackageJson = jest.fn().mockReturnValueOnce(Promise.resolve(packgeJsonInfos))
      utils.getPackageManagerFromLockFile = jest.fn().mockReturnValueOnce('yarn')
      childProcess.execSync.mockReturnValue(
        'https://github.com/kefranabg/readme-md-generator.git'
      )
